locals {
  ...
  sudo az storage blob download -c ${azurerm_storage_container.httpd_files.name} -f /etc/httpd/conf.d/app1.conf -n app1.conf --account-name ${azurerm_storage_account.sa.name} --account-key ${azurerm_storage_account.sa.primary_access_key}
...
resource "azurerm_linux_virtual_machine_scale_set" "web" {
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      application_gateway_backend_address_pool_ids = [azurerm_application_gateway.ag.backend_address_pool[0].id]
    }
  }
